...
def open_file(fpath):
    """Open file as gzip or text depending on content, not filename."""
    raw_file = open(fpath, 'rb')
    start = raw_file.read(2)
    raw_file.seek(0)
    if start == b'\x1f\x8b':  # GZIP magic number
        return gzip.open(raw_file, mode="rt", encoding="utf-8")
    else:
...
def check_consistency(pred_seq, ans_seq, uid):
    error_prefix = f"Error occurring regarding uid {uid}: "
    ...
    for idx, (pred_step, ans_step) in enumerate(zip(pred_seq, ans_seq)):
        ...
        if not (pred_d == ans_d and pred_t == ans_t):
            error(
                error_prefix +
                "Day and time are not the same; "
                f"(d, t) = ({pred_d}, {pred_t}) for generated while (d, t) = ({ans_d}, {ans_t}) for reference at step {idx}.")
...
def main():
    if len(sys.argv) != 4:
        error(
            "Usage: \n"
            "    python3 validator.py task_id dataset_file_path submission_file_path\n"
            "        where task_id is a, b, c, d, or test")
    ...
    uid_min, uid_max = specs["uid_range"]
    uid_set_ref = set(range(uid_min, uid_max + 1))

    uid_set = set()
    pred_uid_dict = dict()

    print("Loading the submission file...")
    with open_file(generated_fpath) as f:
        for i, l in enumerate(f):
            if i == 0 and l.startswith("uid,"):
                continue
            cols = l.rstrip().split(",")

            error_prefix = f"Error at line index {i}: "
            ...
            uid, d, t, x, y = map(int, cols)
            uid_set.add(uid)

            if d < d_min or d > d_max:
                error(error_prefix + f"d={d} is out of range ({d_min}-{d_max})")
            if t < t_min or t > t_max:
                error(error_prefix + f"t={t} is out of range ({t_min}-{t_max})")
            if x < coord_min or x > coord_max:
                error(error_prefix + f"x={x} is out of range ({coord_min}-{coord_max})")
            if y < coord_min or y > coord_max:
                error(error_prefix + f"y={y} is out of range ({coord_min}-{coord_max})")

            if uid not in pred_uid_dict:
                pred_uid_dict[uid] = []
            pred_uid_dict[uid].append((d, t, x, y))

    print("\nChecking the set of uid's...")
    print(f"# of uid's: {len(uid_set)}")
    if uid_set != uid_set_ref:
        error("The set of uid's doesn't match that of reference; there seem to be extra or missing uid's")
    print()
    ...
    for uid in uid_set_ref:
        ...
        if pred_seq is None or ans_seq is None:
            error(f"Missing data for uid {uid}")
        check_consistency(pred_seq, ans_seq, uid)
